...
// Página da sala (funcional com WebSocket)
function RoomPage() {
  ...
  const { sendMessage, isConnected, addMessageHandler } = useWebSocket(wsUrl, userId, roomId, handleWebSocketMessage);
  ...
  const {
    ...
    connectionErrors,
    videoQuality,
  } = useVideoCall({ roomId, userId, sendMessage, addMessageHandler });

  function handleWebSocketMessage(data: any) {
    console.log('[WebSocket] Mensagem recebida:', data);
    
    if (data.type === 'message') {
      setMessages((prev: any[]) => [...prev, data.data]);
    } else if (data.type === 'transcription') {
      setTranscriptions((prev: any[]) => [...prev, data.data]);
